# 🔍 ChatGPT SEO Analyst 
*Unlock AI's Hidden Search Patterns*

## **Want Maximum ChatGPT Visibility?**

**Need your brand to be visible in ChatGPT?**  
**Contact [Unic.com](https://unic.com) - Our specialists optimize for ChatGPT visibility & AI discoverability!**

<div align="center" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; border-radius: 10px;">

### **Expert ChatGPT Visibility Consulting**

*ChatGPT Optimization* • *AI Content Strategy* • *Visibility Audits*

</div>

---

## **What This Extension Does**

Reveals ChatGPT's internal search queries and reasoning patterns in real-time. See exactly what AI searches for when answering questions - giving you unprecedented SEO insights.

**Key Features:**
- **Real AI Search Queries** - See ChatGPT's actual search terms
- **Internal Reasoning** - Understand AI decision-making process  
- **Export Data** - Save insights for analysis
- **Live Overlay** - Non-intrusive interface on ChatGPT
- **Quick Toggle** - `Ctrl+Shift+S` shortcut

## 📥 **How to Install**

### **3 Simple Steps:**

1. **Download** this folder to your computer
2. **Open Chrome** → Go to `chrome://extensions/` → Toggle **Developer mode** ON
3. **Click "Load unpacked"** → Select this folder → Done! ✅

### **How to Use:**
1. Go to **ChatGPT.com**
2. Ask any question
3. Watch the **overlay appear** with AI's internal search queries
4. Press **`Ctrl+Shift+S`** to toggle on/off

## 🎯 Usage

### Basic Usage

1. **Navigate to ChatGPT**: Go to [https://chatgpt.com](https://chatgpt.com)
2. **Start a Conversation**: Ask questions that might trigger search queries
3. **View Analysis**: The overlay will automatically appear when relevant data is detected
4. **Export Data**: Click the export button to copy analysis data to clipboard

### Keyboard Shortcuts

- `Ctrl+Shift+S`: Toggle overlay visibility
- Click extension icon: Open control popup

### Extension Popup

Click the extension icon to access:
- **Status Information**: Extension state, current page, request count
- **Toggle Controls**: Auto-show overlay, network monitoring
- **Action Buttons**: Toggle overlay, clear data
- **Quick Tips**: Helpful usage information

### What Data is Captured

The extension monitors ChatGPT's API responses for:

1. **Search Queries** (`metadata.search_queries`):
   - Actual search terms used by ChatGPT
   - Helpful for understanding SEO opportunities

2. **Internal Thoughts** (`content.thoughts`):
   - ChatGPT's reasoning process
   - Planning and decision-making insights

## 🔧 Configuration

### Auto-show Settings
- **Enabled**: Overlay appears automatically when relevant data is detected
- **Disabled**: Manual toggle required to view overlay

### Network Monitoring
- **Enabled**: Actively monitors ChatGPT API requests
- **Disabled**: Stops monitoring (requires page reload)

## 🎨 Features in Detail

### Modern Overlay Design
- **Dark Theme**: Matches ChatGPT's interface
- **Responsive**: Works on desktop and mobile
- **Non-intrusive**: Positioned to avoid content overlap
- **Smooth Animations**: Slide-in effects and transitions

### Data Export
- **JSON Format**: Structured data export
- **Clipboard Copy**: One-click copying
- **Timestamped**: Includes analysis timestamp and URL

### Error Handling
- **CORS Protection**: Handles network restrictions gracefully
- **API Changes**: Robust error handling for ChatGPT updates
- **Visual Feedback**: Clear error messages and status indicators

## 🔍 How It Works

1. **Background Script** (`background.js`):
   - Uses `chrome.webRequest` API to monitor network traffic
   - Filters for ChatGPT conversation endpoints
   - Coordinates with content script

2. **Content Script** (`content.js`):
   - Injected into ChatGPT pages
   - Fetches and analyzes API responses
   - Creates and manages overlay interface

3. **Network Analysis**:
   - Monitors POST requests to `/backend-api/conversation/`
   - Re-fetches responses to parse JSON data
   - Extracts search queries and thoughts from response structure

## 🐛 Troubleshooting

### Extension Not Working
- Ensure you're on `chatgpt.com`
- Check Developer Mode is enabled
- Reload the extension in `chrome://extensions/`

### No Data Detected
- Try asking questions that require current information
- Some ChatGPT responses don't trigger searches
- Check browser console for error messages

### Network Issues
- CORS restrictions may prevent data access
- Clear browser cache and cookies
- Disable other conflicting extensions

### Overlay Not Appearing
- Check auto-show setting in popup
- Use `Ctrl+Shift+S` to manually toggle
- Verify extension permissions

## 🚨 Important Notes

- **Privacy**: All analysis happens locally in your browser
- **No Data Collection**: Extension doesn't send data anywhere
- **ChatGPT Updates**: May require updates if ChatGPT changes their API
- **Development Extension**: This is a development tool, not for production use

## 🛡️ Security & Privacy

- **Local Processing**: All data analysis happens in your browser
- **No External Requests**: Extension doesn't communicate with external servers
- **Minimal Permissions**: Only requests necessary permissions for functionality
- **No Data Storage**: Doesn't permanently store your conversations

## 📈 SEO Use Cases

### Content Research
- Discover search terms ChatGPT uses for your topics
- Identify knowledge gaps and content opportunities
- Understand semantic relationships

### Keyword Analysis
- See how AI interprets search queries
- Find long-tail keyword variations
- Analyze search intent patterns

### Competitive Analysis
- Understand what information AI prioritizes
- Discover trending topics and searches
- Identify content optimization opportunities

## 🔄 Updates & Maintenance

This extension may need updates if:
- ChatGPT changes their API structure
- Chrome updates break compatibility
- New features or data types are added

Check the project repository for updates and fixes.

## 📄 License

This project is for educational and development purposes. Use responsibly and in accordance with ChatGPT's terms of service.

## 🤝 Contributing

Feel free to contribute improvements, bug fixes, or new features. Please ensure all changes maintain privacy and security standards.

---

**Happy Analyzing!** 🎉

For issues or questions, please check the browser console for error messages and ensure you're using the latest version of Chrome. 